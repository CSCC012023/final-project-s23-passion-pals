...
## Tracking on JIRA  (max 20 marks) 
  ...
  - Do the pull request on GitHub match the tasks listed on JIRA Active Sprint Board? (max 2 marks)
    ...
    Your Mark: 1.5 (description in PR doesn't include changes (ie. events feature not mentioned in the only PR))
  ...
  - Is each pull request made from a separate branch for each individual task? (max 5 marks)
    ...
    Your Mark: 2.5 (make different branches for different features instead of having one huge PR)
  ...
  - Is the git commit message clear? (max 2 marks)
    ...
    Your Mark: 1.5 (most commit messages but missing which ticket is being worked on)
...
Tracking on JIRA Total Mark: 14.5 / 20
...
## Sprint Demo (Max 14 marks) 
  ...
  - Documentation (max 2 marks)
    ...
    Your Mark: 1.5 (add more comments (ie. brief description for frontend components, etc.))
